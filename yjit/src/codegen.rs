...
fn gen_set_ivar(
    ...
    // Call rb_vm_set_ivar_id with the receiver, the index of the ivar, and the value
    let val = asm.ccall(
...
fn gen_get_ivar(
    ...
    let flags_opnd = asm.and(Opnd::UImm(expected_flags_mask as u64), flags_opnd);
        ...
        match def_type {
            ...
            VM_METHOD_TYPE_ATTRSET => {
                if flags & VM_CALL_KWARG != 0 {
                    ...
                    let ivar_name = unsafe { get_cme_def_body_attr_id(cme) };
                    return gen_set_ivar(jit, ctx, asm, ivar_name);
                }
            }
