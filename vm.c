...
vm_exec_handle_exception(rb_execution_context_t *ec, enum ruby_tag_type state, VALUE errinfo)
{
    ...
    for (;;) {
        ...
        while (ec->cfp->pc == 0 || CFP_ISEQ(ec->cfp) == 0) {
        ...
        cfp = ec->cfp;
        epc = cfp->pc - ISEQ_BODY(CFP_ISEQ(cfp))->iseq_encoded;
...
rb_execution_context_update(rb_execution_context_t *ec)
{
    ...
    if (ec->vm_stack) {
        ...
        while (cfp != limit_cfp) {
            ...
            SET_CFP_ISEQ(cfp, (rb_iseq_t *)rb_gc_location((VALUE)CFP_ISEQ(cfp)));
